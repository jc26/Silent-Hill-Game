...
# Pygame Framework created by Lukas Peraza for the 15-112 F15 Pygame Lecture
...
class Game(PygameGame):
    ...
    def monstersDir(self):
        ...
        for monster in self.currRoom.monstersGroup.sprites():
            ...
            if currMove == None: (dx, dy) = (0, 0)
            else: (dx, dy) = currMove
            monster.move = (dx, dy)
            if (dx, dy) == self.dirs[0]: monster.currDir = 'up'
            elif (dx, dy) == self.dirs[2]: monster.currDir = 'right'
            elif (dx, dy) == self.dirs[4]: monster.currDir = 'down'
            elif (dx, dy) == self.dirs[6]: monster.currDir = 'left'
    ...
    def collide2(self):
        if pygame.sprite.groupcollide(self.currRoom.healthGroup,
            ...
            if self.health + 20 > 100: self.health, self.healthRatio = 100, 1
            else: 
                self.health += 20
        ...
        if pygame.sprite.groupcollide(self.currRoom.sanityGroup,
            ...
            if self.sanity + 20 > 100: self.sanity, self.sanityRatio = 100, 1
            else: 
                self.sanity += 10
    ...
    def addHighScore(self):
        ...
        else: newScore = "Survived %d Wave on %s/%s/%s at %s:%s" % (waves, 
                month, day, year, hour, minute)
        ...
        else: Game.writeFile('highScores.txt', currContents + "\n" + newScore)
    ...
    def drawTransWall(self, screen, x, y):
        # draw the transparent box if mouse hovers over walls
    ...
    def changeScreens(self, screen):
        # changing rooms so flash a black screen
        # code adapted from http://stackoverflow.com/questions/6339057/
        # draw-a-transparent-rectangle-in-pygame
        if self.diffScreen:
